...
export async function registerFeedntRecipient(
  ownerKeyId: string,
  username: string,
  ...
  const existing = await loadFeedntUserByUsername(ownerKeyId, trimmedName);
  if (existing) {
  ...
  await saveFeedntUser(ownerKeyId, trimmedName, slimJwk, {
